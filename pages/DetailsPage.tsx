...
const DetailsPage: React.FC = () => {
    ...
    const filteredAlbums = LIBRARY_ALBUMS.filter(album => {
        const matchesStatus = filter === 'all' || album.status === filter;
                ...
                <div className="flex flex-col gap-8 max-w-7xl mx-auto">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                        {filteredAlbums.map(album => (
                            <Link to={`/album/${album.id}`} key={album.id} className="group relative flex flex-col gap-3 p-3 rounded-2xl bg-white dark:bg-surface-dark border border-slate-200 dark:border-border-dark hover:border-primary/50 transition-all hover:shadow-xl hover:-translate-y-1 cursor-pointer">
                                <div className="relative aspect-square rounded-xl overflow-hidden bg-slate-100 dark:bg-black/20">
                                    <div className="absolute inset-0 bg-cover bg-center transition-transform duration-500 group-hover:scale-110" style={{backgroundImage: `url('${album.cover}')`}}></div>
                                    <div className="absolute top-2 right-2">
                                        <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider backdrop-blur-md ${
                                            ...
                                            {album.status === 'listened' ? t('library.filter.listened') : t('library.filter.saved')}
                                        </span>
                                    </div>
                                ...
                                <div className="flex flex-col px-1">
                                    ...
                                    <p className="text-primary text-xs font-medium mt-1">{album.genre}</p>
                                </div>
                            </Link>
                        ))}
                    </div>
